    ...
    >
      ...
      <v-spacer></v-spacer>
       ...
       <template v-slot:extension>
        ...
        >
          <v-tab 
          v-for="item in itemsNavigation" :key="item.title"
        ...
        <v-navigation-drawer v-model="drawer" app width="250">
            <v-list dense class="display-2">
                ...
                :key="item.title" 
                router :to="{name: item.link}">
                <v-list-item v-if="item.list == false" router :to="{name: item.link}">
                  <v-list-item-icon>
    ...
    data () {
        return {
            ...
            itemsNavigation: [
              ...
              { title: 'Presentation', icon: 'mdi-account', link: 'About', list: false},
              { title: 'Réalisations', icon: 'mdi-folder-multiple', link: 'Achievements', list: false},
              { title: 'Prestations', icon: 'mdi-flag', link: 'Services', list: true},
              { title: 'Contact', icon: 'mdi-contacts', link: 'Contact', list: false},
            ],
        }
    },
    computed: {
